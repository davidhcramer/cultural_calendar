...
const RATINGS={ '':'Not rated',loved:'Loved it',liked:'Liked it',mixed:'Mixed',disliked:'Not for me'};
...
function passesReviewFilter(e){if(view==='seen')return choice(e)==='seen';if(view==='return')return returnCandidate(e);if(view==='booking')return choice(e)==='will_book_need_date';if(stillConsidering)return choice(e)==='considering';if(view==='reviewed')return isReviewed(e);return ['shortlist','archive'].includes(view)||!hideReviewed||!isReviewed(e);}
...
function renderControls(){const reviewExempt=['shortlist','booking','reviewed','archive','seen','return','artists'].includes(view);document.getElementById('hide-reviewed').checked=hideReviewed;document.getElementById('hide-reviewed').disabled=reviewExempt||stillConsidering;document.getElementById('still-considering').checked=stillConsidering;document.getElementById('still-considering').disabled=['booking','seen','return'].includes(view);document.getElementById('review-filter-note').textContent=view==='booking'?'Committed to going; choose a date and book.':stillConsidering?'Showing saved decisions marked Considering.':reviewExempt?'This view includes saved decisions.':'Save a decision to clear it from this view.';const cats={all:'All four strands',...LABELS};document.getElementById('category-filters').innerHTML=Object.entries(cats).map(([key,label])=>`<button data-category="${key}" class="${key===category?'active':''}" aria-pressed="${key===category}">${label}<span>${events.filter(e=>(view==='archive'?!active(e):['reviewed','seen','return'].includes(view)||active(e))&&passesReviewFilter(e)&&(key==='all'||e.category===key)).length}</span></button>`).join('');document.getElementById('view-filters').innerHTML=Object.entries({all:'Forward diary',daily:'Calendar by day',shortlist:'My shortlist',booking:'Choose dates',reviewed:'Reviewed',seen:'Past visits / notes',return:'Go again',artists:'Artists to follow',closing:'Closing within 30 days',new:'New this run',watchlist:'Dates unannounced',archive:'Past / withdrawn'}).map(([key,label])=>`<button data-view="${key}" class="${view===key?'active':''}" aria-pressed="${view===key}">${label}</button>`).join('');}
...
function likelyRepeatable(e){return Boolean(e.series_id||e.recurrence||/open studios?|art trail|art fair|\bfair\b|festival|frieze london|frieze masters|pad london|summer intensive|open house/i.test(e.title));}
function positiveVisit(c){if(c.visit_rating)return ['loved','liked'].includes(c.visit_rating);const n=c.visit_notes||'';return !/\b(not|never|didn.t|wasn.t|disappoint|avoid)\b/i.test(n)&&/\b(loved|excellent|fantastic|really enjoyed|would go again)\b/i.test(n);}
function returnCandidate(e){const c=choices[e.id]||{};return c.return_interest==='watch'||(c.status==='seen'&&c.return_interest!=='dismissed'&&likelyRepeatable(e)&&positiveVisit(c));}
function returnEvidence(e){const last=(DATA.return_checks||[]).filter(c=>c.previous_event_id===e.id).sort((a,b)=>b.checked_on.localeCompare(a.checked_on))[0];if(!last)return '';return `<p class="small">Next edition checked ${esc(last.checked_on)}: ${esc(last.notes||last.status)} ${(last.next_event_ids||[]).filter(id=>byId.has(id)).map(id=>`<button class="inline-link" data-open-event="${esc(id)}">${esc(byId.get(id).title)}</button>`).join(' · ')} ${(last.source_urls||[]).map((u,i)=>link(u,'Evidence '+(i+1))).join(' · ')}</p>`;}
function returnFields(e,state){const suggested=likelyRepeatable(e)&&positiveVisit(state)&&!state.return_interest;return `<div class="return-fields"><label for="rating-${e.id}">Your verdict</label><select id="rating-${e.id}" data-visit-rating="${e.id}">${Object.entries(RATINGS).map(([v,t])=>`<option value="${v}" ${state.visit_rating===v?'selected':''}>${t}</option>`).join('')}</select><label for="return-${e.id}">Next edition</label><select id="return-${e.id}" data-return-interest="${e.id}"><option value="" ${!state.return_interest?'selected':''}>Decide later</option><option value="watch" ${state.return_interest==='watch'?'selected':''}>Follow the next edition</option><option value="dismissed" ${state.return_interest==='dismissed'?'selected':''}>Don’t suggest again</option></select>${suggested?'<p class="small return-suggestion">You enjoyed this and it may repeat. It’s flagged in Go again for the next refresh.</p>':''}<label for="series-${e.id}">Series or organiser (optional)</label><input id="series-${e.id}" data-return-series="${e.id}" value="${esc(state.return_series||'')}" placeholder="For example, APT Open Studios"><label for="season-${e.id}">When to look again (optional)</label><input id="season-${e.id}" data-return-season="${e.id}" value="${esc(state.return_season||'')}" placeholder="Next autumn, spring season…"><p class="small">Next-edition dates stay unconfirmed until an announcement is found.</p></div>`;}
function visitSummary(e){const c=choices[e.id]||{};return `<div class="visit-summary">${c.visited_on?'Visited '+esc(fullFormat.format(parseDate(c.visited_on))):'Visit date not recorded'}${c.visit_rating?' · '+esc(RATINGS[c.visit_rating]||''):''}${returnCandidate(e)?' · '+(c.return_interest==='watch'?'Following next edition':'Suggested for next edition'):''}${c.visit_notes?`<p>${esc(c.visit_notes)}</p>`:''}</div>`;}
function eventLocations(e){return Array.isArray(DATA.locations?.[e.id])?DATA.locations[e.id]:[];}
function locationSearch(e){return eventLocations(e).map(p=>[p.name,p.address,p.neighbourhood].filter(Boolean).join(' ')).join(' ');}
function mapsURL(e,place){const query=[place.name||e.venue,place.address,place.city||e.city,place.country||e.country].filter(Boolean).join(', ');return 'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(query);}
function locationHTML(e){
  const places=eventLocations(e);
  if(!places.length)return `<div class="event-location">${link(mapsURL(e,{}),'Find venue on Google Maps')}</div>`;
  return places.map(place=>{
    const area=place.neighbourhood?` (${esc(place.neighbourhood)})`:'';
    if(place.scope==='area')return `<div class="event-location">Several locations${area} · ${link(place.programme_url||e.booking_url||e.source_urls[0],'See programme')}</div>`;
    return `<div class="event-location">${link(mapsURL(e,place),place.address||place.name||e.venue)}${area}</div>`;
  }).join('');
}
function venueName(e){if(/room|wing|playhouse|annexe|project space|gallery i|drawings/i.test(e.venue))return e.venue;const places=eventLocations(e);return places.length===1&&places[0].name?places[0].name:e.venue;}
function eventHTML(e,attention=[]){const saved=choices[e.id]||{status:'unreviewed',notes:''},state=drafts[e.id]||saved;renderedChoices[e.id]={...saved};let tags=[decisionBadge(saved.status),...attention.map(({date,label})=>`<span class="tag warn"><time datetime="${esc(date)}">${dateFormat.format(parseDate(date))}</time>: ${esc(label)}</span>`),`<span class="tag">${esc(LABELS[e.category])}</span>`];if(e.access_note)tags.push(`<span class="tag warn">${esc(e.access_note)}</span>`);if(e.highlight_start_date)tags.push(`<span class="tag warn">Key works: ${dateFormat.format(parseDate(e.highlight_start_date))}–${dateFormat.format(parseDate(e.highlight_end_date))}</span>`);if(age(e)>14)tags.push(`<span class="tag warn">Checked ${age(e)} days ago</span>`);if(e.status==='watchlist')tags.push('<span class="tag warn">Watchlist</span>');if(['cancelled','superseded'].includes(e.status))tags.push(`<span class="tag warn">${esc(e.status)}</span>`);return `<details class="event" id="event-${e.id}"><summary><div class="event-date">${range(e)}</div><div class="event-heading"><h3>${esc(e.title)}</h3><div class="venue">${esc(venueName(e))} · ${esc(e.city)}</div>${locationHTML(e)}${['seen','return'].includes(view)?visitSummary(e):''}<div class="tags">${tags.join('')}</div></div><div class="priority"><b>${e.priority}</b><span class="word">${{A:'Decide',B:'Consider',C:'Explore'}[e.priority]}</span></div></summary><div class="event-body"><p class="event-overview">${esc(e.overview||e.priority_reason||'')}</p><p><strong>Next step.</strong> ${esc(e.action)}</p>${e.action_date?`<p><strong>${fullFormat.format(parseDate(e.action_date))}:</strong> ${esc(e.action_label)}</p>`:''}<p><strong>Booking.</strong> ${esc(e.booking_status)} ${e.booking_url?link(e.booking_url,'Check official page'):''}</p><p><strong>Cost.</strong> ${esc(e.cost_note)}</p>${e.notes?`<p class="small">${esc(e.notes)}</p>`:''}<p class="source-links">${e.source_urls.map((u,i)=>link(u,`Source ${i+1}`)).join(' · ')}. Checked ${esc(e.verified_on)}.</p><div class="decision"><div><label for="choice-${e.id}">Your decision</label><select id="choice-${e.id}" data-choice="${e.id}">${Object.entries(DECISIONS).map(([k,v])=>`<option value="${k}" ${k===state.status?'selected':''}>${v}</option>`).join('')}</select></div><div><label for="notes-${e.id}">Planning notes</label><textarea id="notes-${e.id}" data-notes="${e.id}" placeholder="A date to try, someone to go with, what caught your eye">${esc(state.notes)}</textarea></div>${['seen','return'].includes(view)?`<div class="visit-fields"><label for="visit-date-${e.id}">Date attended (optional)</label><input id="visit-date-${e.id}" type="date" data-visit-date="${e.id}" value="${esc(state.visited_on||'')}" max="${todayISO}"><label for="visit-notes-${e.id}">After your visit</label><textarea id="visit-notes-${e.id}" data-visit-notes="${e.id}" placeholder="What stayed with you? Favourite works, discoveries, things to revisit.">${esc(state.visit_notes||'')}</textarea>${returnFields(e,state)}${returnEvidence(e)}</div>`:''}<div class="decision-actions"><button class="decision-save" data-save-choice="${e.id}">Save</button><span class="small" id="draft-status-${e.id}" role="status">${drafts[e.id]?'Unsaved changes. Click Save when ready.':'Choose a decision, add any notes, then Save.'}</span></div></div></div></details>`;}
function filtered(){const term=document.getElementById('search').value.toLocaleLowerCase(),priority=document.getElementById('priority').value,month=document.getElementById('month').value;return events.filter(e=>{if(!passesReviewFilter(e))return false;if(category!=='all'&&e.category!==category)return false;if(priority!=='all'&&e.priority!==priority)return false;if(term&&!JSON.stringify([e.title,e.venue,e.city,e.country,locationSearch(e),e.overview,e.personal_fit,e.notes,e.tags,choices[e.id]?.notes,choices[e.id]?.visit_notes]).toLocaleLowerCase().includes(term))return false;if(view==='archive'){if(active(e))return false;}else if(!['reviewed','seen','return'].includes(view)&&!active(e))return false;if(view==='shortlist'&&!['considering','will_book_need_date','booked'].includes(choice(e)))return false;if(view==='closing'&&(!e.end_date||parseDate(e.end_date)-parseDate(asOf)>30*DAY))return false;if(view==='new'&&!DATA.report.new_ids.includes(e.id))return false;if(view==='watchlist'&&e.start_date)return false;if(month!=='all'){if(['seen','return'].includes(view)){if(!choices[e.id]?.visited_on?.startsWith(month))return false;}else if(!e.start_date||e.start_date.slice(0,7)>month||e.end_date.slice(0,7)<month)return false;}return true;}).sort((a,b)=>{if(['seen','return'].includes(view))return (choices[b.id]?.visited_on||'').localeCompare(choices[a.id]?.visited_on||'')||a.title.localeCompare(b.title);const aKey=a.start_date?(a.start_date<asOf&&a.end_date>=asOf?'0000':a.start_date):'9999';const bKey=b.start_date?(b.start_date<asOf&&b.end_date>=asOf?'0000':b.start_date):'9999';return aKey.localeCompare(bKey)||a.priority.localeCompare(b.priority)||a.title.localeCompare(b.title);});}
...
function render(){
  ...
  document.getElementById('month').setAttribute('aria-label',['seen','return'].includes(view)?'Visit month':'Month');
  ...
  if(view==='seen')document.getElementById('result-count').textContent=`${visible.length} visits. Latest visits first; undated visits stay at the end.`;
  if(view==='return')document.getElementById('result-count').textContent=`${visible.length} events to revisit. Followed editions and suggestions are checked during refreshes; no future dates are assumed.`;
  const attention=['reviewed','archive','seen','return'].includes(view)?[]:visible.map(e=>({e,dates:attentionDates(e)})).filter(item=>item.dates.length).sort((a,b)=>a.dates[0].date.localeCompare(b.dates[0].date)||a.e.priority.localeCompare(b.e.priority)||a.e.title.localeCompare(b.e.title));
  ...
  for(const e of visible){
    ...
    const g=['seen','return'].includes(view)?(choices[e.id]?.visited_on?new Intl.DateTimeFormat('en-GB',{month:'long',year:'numeric',timeZone:'UTC'}).format(parseDate(choices[e.id].visited_on)):'Visit date not recorded'):!e.start_date?'Dates unannounced':e.start_date<asOf&&e.end_date>=asOf?'On now':new Intl.DateTimeFormat('en-GB',{month:'long',year:'numeric',timeZone:'UTC'}).format(parseDate(e.start_date));
...
document.addEventListener('click',event=>{const c=event.target.closest('[data-category]'),v=event.target.closest('[data-view]'),detail=event.target.closest('[data-open-event]'),save=event.target.closest('[data-save-choice]');if(save){commitChoice(save.dataset.saveChoice);return;}if(c){category=c.dataset.category;render();}if(v){view=v.dataset.view;if(['booking','seen','return'].includes(view)){stillConsidering=false;try{localStorage.setItem(CONSIDERING_STORE,'false');}catch(e){}}if(['watchlist','reviewed','booking','seen','return'].includes(view))document.getElementById('month').value='all';render();}if(detail)showEvent(detail.dataset.openEvent);});
...
function draftChoice(event){
  const id=event.target.dataset.choice||event.target.dataset.notes||event.target.dataset.visitDate||event.target.dataset.visitNotes||event.target.dataset.visitRating||event.target.dataset.returnInterest||event.target.dataset.returnSeries||event.target.dataset.returnSeason;if(!id||!byId.has(id))return;
  ...
  if(event.target.dataset.choice){if(!Object.hasOwn(DECISIONS,event.target.value))return;draft.status=event.target.value;}else if(event.target.dataset.visitDate)draft.visited_on=event.target.value;else if(event.target.dataset.visitNotes)draft.visit_notes=event.target.value;else if(event.target.dataset.visitRating)draft.visit_rating=event.target.value;else if(event.target.dataset.returnInterest)draft.return_interest=event.target.value;else if(event.target.dataset.returnSeries)draft.return_series=event.target.value;else if(event.target.dataset.returnSeason)draft.return_season=event.target.value;else draft.notes=event.target.value;
...
async function commitChoice(id){
  ...
  next.return_candidate=next.return_interest==='watch'||(next.status==='seen'&&next.return_interest!=='dismissed'&&likelyRepeatable(byId.get(id))&&positiveVisit(next));
  ...
  try{
    ...
    document.getElementById('review-feedback').textContent='Saved: '+byId.get(id).title+'.'+(returnCandidate(byId.get(id))?' Flagged in Go again for the next refresh.':'');
    return true;
  }catch(error){
...
function findingGroup(f){
  if(f.kind==='sale')return 'Market prices & sales';
  if(f.kind!=='exhibition')return 'News';
  const e=byId.get(f.event_id),start=f.start_date||f.date||e?.start_date,end=f.end_date||e?.end_date;
  if(end&&end<asOf)return 'Recent events';
  if(start&&start>asOf)return 'Upcoming events';
  if(start&&end&&start<=asOf&&end>=asOf)return 'On now';
  return 'Events with incomplete dates';
}
function priceHTML(f){
  const p=f.price;if(!p||typeof p!=='object')return '<span class="small">No public price recorded.</span>';
  const types={asking:'Asking price',realised:'Realised sale',estimate:'Auction estimate'};
  const value=p.amount??(p.low!=null&&p.high!=null?`${p.low}–${p.high}`:null);
  if(value==null||!p.currency||!types[p.type])return '<span class="small">Price details incomplete.</span>';
  return `<span class="price-evidence">${esc(types[p.type])}: ${esc(p.currency)} ${esc(value)}${p.date?' · '+esc(p.date):' · date not recorded'}</span><span class="small">${[p.medium,p.dimensions,p.edition,p.fees_note||(p.type==='realised'?'Buyer’s premium inclusion unknown':'')].filter(Boolean).map(esc).join(' · ')}</span>`;
}
function artistFindings(checks){
  const latest=new Map();
  for(const c of checks)for(const f of c.findings||[]){const key=f.id||[f.kind,f.event_id||f.url,f.title].join('|');if(!latest.has(key))latest.set(key,{...f,checked_on:c.checked_on});}
  const groups=['Upcoming events','On now','Recent events','Events with incomplete dates','News','Market prices & sales'];
  return groups.map(group=>{
    const findings=[...latest.values()].filter(f=>findingGroup(f)===group).sort((a,b)=>group==='Upcoming events'?(a.start_date||a.date||'9999').localeCompare(b.start_date||b.date||'9999'):(b.end_date||b.date||b.checked_on||'').localeCompare(a.end_date||a.date||a.checked_on||''));
    return `<section class="artist-findings"><h4>${group}</h4>${findings.length?findings.map(f=>`<div class="artist-finding"><p>${link(f.url,f.title)}${f.event_id&&byId.has(f.event_id)?` · <button class="inline-link" data-open-event="${esc(f.event_id)}">View event</button>`:''}</p><p class="small">${[f.venue,f.city,f.start_date||f.date,f.end_date&&'to '+f.end_date].filter(Boolean).map(esc).join(' · ')} · Checked ${esc(f.checked_on)}</p>${f.summary?`<p>${esc(f.summary)}</p>`:''}${f.kind==='sale'?priceHTML(f):''}</div>`).join(''):'<p class="small">No findings recorded yet.</p>'}</section>`;
  }).join('');
}
